...
// phpcs:ignorefile
class ListingGeocodingTest extends TestCase
{
    /**
    * @test
    */
    ...
    protected function assertLocationsAreGeocoded(Collection $locations)
    {
        ...
        $locations->each(function ($item) use ($user) {
            $listing = factory(Listing::class)->create([
                'state' => $item->get('abbreviation'),
                'zip_code' => $item->get('zipcode'),
                'user_id' => $user->id,
            ]);

            $address = "{$item->get('city')} {$item->get('name')}, {$item->get('zipcode')}";
            $geoloc = $listing->toSearchableArray()['_geoloc'] ?? null;
