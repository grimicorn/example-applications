...
// phpcs:ignorefile
class GeocodeMapquestTest extends TestCase
{
    use RefreshDatabase;
    ...
    */
    public function it_geocodes_past_issue_zipcodes()
    {
        ...
        $this->assertLocationsAreGeocoded($locations);
    }
    ...
    protected function assertLocationsAreGeocoded(Collection $locations)
    {
        ...
        $locations->each(function ($item) use ($user) {
            $listing = factory(Listing::class)->create([
                'city' => $city = $item->get('city'),
                'state' => $stateAbbreviation = $item->get('abbreviation'),
                'zip_code' => $zipCode = $item->get('zipcode'),
                'user_id' => $user->id,
            ]);

            $location = (new Mapquest)->getLocationByZipCode(
                $item->get('zipcode')
            );

            $address = "{$city} {$item->get('name')}, {$zipCode}";
            $this->assertNotNull($location, "Geocode result is null for {$address}");
            $this->assertNotEmpty(2, $location, "Geocode result is missing for {$address}");
            $this->assertEquals($zipCode, $location->get('postalCode'), "Zip code did not match for {$address}");
            $this->assertEquals($stateAbbreviation, $location->get('adminArea3'), "State did not match for {$address}");
        });
    }
