module.exports = {
    ...
    methods: {
        ...
        subscribe() {
            ...
            Stripe.card.createToken(payload, (status, response) => {
                if (response.error) {
                    ...
                    window.Bus.$emit('payment-token-error', response);
        ...
        createSubscription(token) {
            let form = this.form;
            form.stripe_token = token;
            form.card_name = this.cardForm.name;
            form.card_expiration_year = this.cardForm.year;
            form.card_expiration_month = this.cardForm.month;

            Spark.post(this.urlForNewSubscription, form).then((response) => {
                Bus.$emit('updateUser');
                Bus.$emit('updateTeam');
            });
        },
         ...
         */
        hasSubscribed(plan) {
            return !!_.where(this.billable.subscriptions, {
    ...
    computed: {
        ...
        urlForNewSubscription() {
            return this.billingUser
                ...
                : `/settings/${Spark.pluralTeamString}/${
                      this.team.id
                  }/subscription`;
        },
