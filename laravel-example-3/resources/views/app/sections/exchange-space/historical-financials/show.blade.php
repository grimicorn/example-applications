    ...
    @component('app.sections.shared.action-bar')
        @slot('left')
            @if($space->current_member->is_seller)
                <app-exchange-space-toggle-access
                            name="historical_financial_access"
                            ...
                            route="{{ route('exchange-spaces.historical-financials.update', ['id' => $space->id]) }}"
                            :value="{{
        ...
        @slot('right')
            ...
            <app-sort-table-export-button
                route="{{
                    route('exchange-spaces.historical-financials.csv.index', [
                        'id' => $space->id,
                    ])
                }}"
